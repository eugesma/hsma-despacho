class EntryNote < ApplicationRecord
  ...
  def unique_note_number
    if self.nota?
      ...
      if self.new_record?
        return unless EntryNote.exists?(note_number: note_number, created_at: range)
      else
        return unless EntryNote.where.not(id: self.id).exists?(note_number: note_number, created_at: range)
      end
  ...
  def unique_zonal_pass
    if self.pase?
      range = Time.now.beginning_of_year..Time.now.end_of_year
      if self.new_record?
        return unless EntryNote.exists?(zonal_pass: zonal_pass, created_at: range)
      else
        return unless EntryNote.where.not(id: self.id).exists?(zonal_pass: zonal_pass, created_at: range)
      end
